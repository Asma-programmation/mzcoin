<?php session_start(); ob_start(); //on enregistre la session, pour le code, pour la vérification du formulaire
...
$largeur = 120; // largeur de l'image
$hauteur = 25; // hauteur de l'image
$lignes  = 10; // nombre de lignes multicolore qui seront affichées avec le code (10 est bien)
$caracteres = substr(md5(sha1(date("Y-m-d H:i:s.u"))),0,6); // "ABCDEF123456789";//type de caractère du code qui sera affiché dans l'image
...
for($i=0;$i<=$lignes;$i++){
    ...
    $rgb=hexargb(substr(md5(sha1(date("Y-m-d H:i:s.u"))),0,6));//choisi une couleur aléatoirement (str_shuffle), de 6 caractères (substr(chaine,0,6)) avec la sélection alphanumérique
...
$code1 = strtoupper(substr(str_shuffle($caracteres),0,6));
$_SESSION[_treatments::get_string("captcha")]=$code1;//on enregistre le code dans une session pour vérifier ensuite se qu'à entré le visiteur est identique
